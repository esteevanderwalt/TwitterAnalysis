...
sql1 <- paste("SELECT U.ID, U.SCREENNAME, U.LONGITUDE AS LONG1, U.LATITUDE AS LAT1, L.\"lon\" AS LONG2, L.\"lat\" AS LAT2, UPPER(TRIM(U.LOCATION)) LOCATION FROM ",table1," U JOIN TWITTER.SMP_LOCATION_R L ON UPPER(TRIM(L.\"location\")) = UPPER(TRIM(U.LOCATION)) WHERE U.LATITUDE IS NOT NULL AND L.\"lat\" IS NOT NULL
",sep="")
