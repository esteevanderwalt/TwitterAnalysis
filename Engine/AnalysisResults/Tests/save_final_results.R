...
myconn<-odbcConnect("SAPHANA", uid="SYSTEM", pwd="oEqm66jccx", believeNRows=FALSE, rows_at_time=1, DBMSencoding="UTF-8") 
sqlSaveTable <- "TWITTER.ZZ_FINAL_RESULTS_20171208"

#write data back to DB
sqlSave(channel=myconn, data.o, tablename=sqlSaveTable, append=TRUE, rownames=FALSE)  

#scale and center data
data.scaled <- as.data.frame(scale(data.o[4:21], center = TRUE, scale = TRUE))
#add class back
data.scaled <- cbind(data.scaled,data.o[,1:3])

#write back to DB
sqlSaveTable <- "TWITTER.ZZ_FINAL_RESULTS_SCALED_20171208"

sqlSave(channel=myconn, data.scaled, tablename=sqlSaveTable, append=TRUE, rownames=FALSE)  
