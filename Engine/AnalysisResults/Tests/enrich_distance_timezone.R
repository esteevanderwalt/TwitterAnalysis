...
sql1 <- paste("SELECT U.ID, U.SCREENNAME, L.\"lon\" AS LONG1, L.\"lat\" AS LAT1, TZ.\"lon\" AS LONG2, TZ.\"lat\" AS LAT2 FROM ",table1," U JOIN TWITTER.SMP_LOCATION_R L ON UPPER(TRIM(L.\"location\")) = UPPER(TRIM(U.LOCATION)) AND L.\"lat\" IS NOT NULL JOIN TWITTER.SMP_LOCATION_R TZ ON UPPER(TRIM(TZ.\"location\")) = UPPER(TRIM(U.TIMEZONE)) AND TZ.\"lat\" IS NOT NULL WHERE U.TIMEZONE IS NOT NULL AND U.LOCATION IS NOT NULL
",sep="")
