...
getRandomData <- function(d, si){
  ...
  decept <- deceptive[sample(nrow(deceptive), si), ]
  #add to deceptive
  total <- rbind(decept, trustworthy)
  #return
  return(total)
}
...
tune <- c(3)
#sampling
sampling <- c("smote")  #"smote", "rose", "up", 
#summary function
summF <- c("prSummary") #"twoClassSummary", 
rr <- c(1)  
sz <- c(100,500,1000,5000,10000) #use 0 for full set - , 1000, 10000, 100000
    ...
    for (x in folds) {
      for (y in repeats) {
        for (z in tune) {
          for (s in sz) {
            for (r in 1:1) {  #rr
              filename <- paste("~/Projects/RStudio/TwitterAnalysis/Engine/AnalysisResults/Results/META15K_progSamp_",x,"fold_",y,"repeat_",z,"tune_",m,"_sumf_",n,"_size_",s,"_round_",r,".txt",sep="")
              imagefilename <- paste("~/Projects/RStudio/TwitterAnalysis/Engine/AnalysisResults/Results/META15K_progSamp_",x,"fold_",y,"repeat_",z,"tune_",m,"_sumf_",n,"_size_",s,"_round_",r,"_",sep="")
